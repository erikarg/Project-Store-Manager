...
const { rightProductBody, productCreateResponse, wrongProductBody, wrongSizeProductBody } = require('../../../__tests__/_dataMock');
...
describe("Testes de unidade das validações de input", function () {
  it("Testa se é possível listar os produtos por id", async function () {
    sinon.stub(inputsValidations, 'idValidation').resolves({ status: null, message: '' });
  ...
  it("Testa se é possível listar os produtos por nome", async function () {
    sinon.stub(productsService, 'getRegisteredProduct').resolves(productCreateResponse);
  ...
  it("Testa se é possível validar os produtos", async function () {
    sinon.stub(productsModel, 'listProductsById').resolves(productCreateResponse);
    const result = await inputsValidations.productValidation(productCreateResponse);
    expect(result.status).to.equal(null);
    expect(result.message).to.equal('');
    });
